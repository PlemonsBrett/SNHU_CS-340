"""
This type stub file was generated by pyright.
"""

import typing
import numbers
from typing_extensions import NotRequired, TypedDict
from dash.development.base_component import Component, ComponentType, _explicitize_args

class Fade(Component):
    """A Fade component.
Hide or show content with a fading animation. Visibility of the children is
controlled by the `is_open` prop which can be targetted by callbacks.

Keyword arguments:

- children (a list of or a singular dash component, string or number; optional):
    The children of the Fade.

- id (string; optional):
    The ID of the Fade.

- is_in (boolean; optional):
    Controls whether the children of the Fade component are currently
    visible or not.

- class_name (string; optional):
    Additional CSS classes to apply to the Fade.

- timeout (dict; optional):
    The duration of the transition, in milliseconds.  You may specify
    a single timeout for all transitions like: `timeout=500` or
    individually like: timeout={'enter': 300, 'exit': 500}.

    `timeout` is a number | dict with keys:

    - enter (number; optional)

    - exit (number; optional)

- appear (boolean; optional):
    Show fade-in animation on initial page load. Default: True.

- enter (boolean; optional):
    Enable or disable enter transitions. Default: True.

- exit (boolean; optional):
    Enable or disable exit transitions. Default: True.

- tag (string; optional):
    HTML tag to use for the fade component. Default: div.

- key (string; optional):
    A unique identifier for the component, used to improve performance
    by React.js while rendering components  See
    https://react.dev/learn/rendering-lists#why-does-react-need-keys
    for more info.

- className (string; optional):
    **DEPRECATED** Use `class_name` instead.  Additional CSS classes
    to apply to the Fade."""
    _children_props = ...
    _base_nodes = ...
    _namespace = ...
    _type = ...
    Timeout = TypedDict("Timeout", { "enter": NotRequired[typing.Union[int, float, numbers.Number]],"exit": NotRequired[typing.Union[int, float, numbers.Number]] })
    @_explicitize_args
    def __init__(self, children: typing.Optional[typing.Union[str, int, float, ComponentType, typing.Sequence[typing.Union[str, int, float, ComponentType]]]] = ..., id: typing.Optional[typing.Union[str, dict]] = ..., *, is_in: typing.Optional[bool] = ..., style: typing.Optional[typing.Any] = ..., class_name: typing.Optional[str] = ..., timeout: typing.Optional[typing.Union[typing.Union[int, float, numbers.Number], Timeout]] = ..., appear: typing.Optional[bool] = ..., enter: typing.Optional[bool] = ..., exit: typing.Optional[bool] = ..., tag: typing.Optional[str] = ..., key: typing.Optional[str] = ..., className: typing.Optional[str] = ..., **kwargs) -> None:
        ...
    


